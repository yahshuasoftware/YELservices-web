    ...
    newCertificates.forEach(newCert => {
      ...
      if (!exists) {
        updatedCertificates.push({
          ...
          proofOfIdentity: newCert.proofOfIdentity,
          proofOfAddress: newCert.proofOfAddress,
          amount: newCert.amount // Add amount field while adding the new certificate
        });
      }
    });
...
const getProofOfDocuments = async (req, res) => {
  ...
  try {
    ...
    // Extract proof of identity, proof of address, and amount
    const proofOfIdentity = certificate.proofOfIdentity || [];
    const proofOfAddress = certificate.proofOfAddress || [];
    const amount = certificate.amount || null; // Include the amount field

    // Send the response with POI, POA, and amount
