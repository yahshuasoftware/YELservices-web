  ...
  return (
    <div className={`${isDarkMode ? "dark" : ""} flex`}>
      ...
      <nav className={`bg-white dark:bg-ocean-800 min-h-screen w-${isClosed ? "20" : "64"} p-4 transition-width duration-300`}>
        <header className="relative flex items-center justify-between">
          ...
          <button onClick={toggleSidebar} className="bg-zinc-600 text-white w-10 p-2 rounded-full">
            <BiChevronRight className={`transform w-5 h-5 ${isClosed ? "" : "rotate-180 w-5 h-5" }`}  />
          </button>
        </header>
        ...
        <div className="mt-auto">
          <button onClick={handleLogout} className="flex items-center space-x-4 w-full py-2 text-md font-medium text-ocean-800 dark:text-ocean-100">
      ...
      <section className="flex-1 p-6">
        ...
        <div>
        <h1 className="text-2xl font-bold"> WELC0ME,{user.email}</h1>
          </div>
